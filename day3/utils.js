...
const makePath = (dirs, startPoint=[0, 0]) => {
    ...
    for (let d of dirs) {
        ...
        const from = path[path.length - 1].join(',').split(',').map(i => parseInt(i));
        switch(direction) {
            case 'U': path = path.concat(makePathUp(from, distance)); break;
            case 'D': path = path.concat(makePathDown(from, distance)); break;
            case 'R': path = path.concat(makePathRight(from, distance)); break;
            case 'L': path = path.concat(makePathLeft(from, distance)); break;
        }
