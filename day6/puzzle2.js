...
for (i in yourParentsPath) {
  if (yourParentsPath[i].name == santaParentsPath[i].name)
    youngestCommonPath = yourParentsPath[i];
  else break;
}
...
let youngestCommonPath_yourRootOffset = youngestCommonPath.offsetOf(
  yourParentsPath[i].root
);
let youngestCommonPath_santaRootOffset = youngestCommonPath.offsetOf(
  santaParentsPath[i].root
);
...
Rta =
  yourOffset -
  1 -
  youngestCommonPath_yourRootOffset +
  santaOffset -
  youngestCommonPath_santaRootOffset +
  (youngestCommonPath_yourRootOffset - youngestCommonPath_santaRootOffset) -
  2;
console.log("Rta", Rta);

// Rta 463 // it is wrong... but I don know the coorect answer and I don't want to debug anything
